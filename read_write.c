...
enum operator {
    EMPTY,READ, WRITE, ASSIGN, MOVE, LOAD, STORE, ADD, MINUS, MULT, MOD, EQ, LESS, JUMP, JUMPIF, DIV, TERM
};
...
int main(int argc, char *argv[]) {
    ...
    while (true)
    {
                ...
                // unsigned char a = 0x0;
                // scanf("%hhu", &a);
                // memArray[inst.r1].num = a;
                // char* temp = NULL;
                // sscanf("\"%s\"", temp);
                // printf("here : %d\n", atoi_v2(temp));
                // memArray[inst.r1].num = atoi_v2(temp);
        struct instruction inst = memArray[pc].inst;
        pc++;
        char input_str[5]; 
        int input;
        switch (inst.op)
        {
            case READ:
                printf("Enter a value: ");
                scanf("%d", &input);
                memArray[inst.r1].num = input;
                printf("here : %d\n", memArray[inst.r1].num);
                break;
            ...
            case ASSIGN:
                memArray[inst.r1].num = inst.r2;
                break;

            case MOVE:
                memArray[inst.r1].num = memArray[inst.r2].num;
                break;

            case LOAD:
                memArray[memArray[inst.r2].num].num = memArray[inst.r1].num; 
                break;

            case STORE:
                memArray[memArray[inst.r1].num].num = memArray[inst.r2].num; 
                break;

            case ADD:
                memArray[inst.r1].num = memArray[inst.r2].num + memArray[inst.r3].num;
                break;
            ...
            case EQ:
                if (memArray[inst.r2].num == memArray[inst.r3].num) {
                    memArray[inst.r1].num = 1;
                }
                else {
                    memArray[inst.r1].num = 0;
                }
                break;
